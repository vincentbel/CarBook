...
class SpCarSeable implements Comparable<SpCarSeable>{
	...
	public int getMatchCounter(){return matchCounter;}
	@Override
...
class PinyinSearch{
	...
	//ËÑË÷????ÓÃµÄº¯Êý¡£ÐèÒª´«Èë³µÁ¾Æ·ÅÆArrayList , ¹Ø¼ü??
	//Êä³ö³µÁ¾ÁÐ±í
	static public ArrayList<CarInfor> search(ArrayList<CarSeable> cslist,String sysmbol){
		...
		if(tmp.length==1){
			//±©Á¦ËÑË÷??
			//´ýÕå??
			for(CarSeable cs : cslist){
				returnlist.addAll(cs.getCarList());
			}
			returnlist = searchCarInfo(returnlist, sysmbol);
			
		}
		else if (tmp.length==2){	//	Æ·ÅÆ+³µÁ¾
			
			ArrayList<CarSeable> al_cs = searchCarSeable(cslist, tmp[0]);
			//ËÑË÷µ½µÄµÚÒ»¸öÆ·??
	...
	static private ArrayList<CarSeable> searchCarSeable(ArrayList<CarSeable> cslist,String sysmbol){
	...
	static private ArrayList<CarInfor> searchCarInfo(ArrayList<CarInfor> cslist,String sysmbol){
		ArrayList<CarInfor> returnlist=new ArrayList<CarInfor>();
		ArrayList<SpCarInfo> tmplist = new ArrayList<SpCarInfo>();
		for(CarInfor cs : cslist){
