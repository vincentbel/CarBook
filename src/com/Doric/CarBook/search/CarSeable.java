...
class CarSeable{
	...
	public void LoadCar(){
		...
		if(carSeableName.equals("大众")){
			for(int i=1;i<10;i++){
				carList.add(new CarInfor(i, "Z", "test", "test", 200.0, "D"+i, 20.0, 100.0, 200.0));
				count++;
			}
		}else if(carSeableName.equals("福特")){
			for(int i=1;i<10;i++){
				carList.add(new CarInfor(i, "Z", "test", "test", 200.0, "F"+i, 20.0, 100.0, 200.0));
				count++;
			}
		}else if(carSeableName.equals("宝马")){
			for(int i=1;i<10;i++){
				carList.add(new CarInfor(i, "Z", "test", "test", 200.0, "B"+i, 20.0, 100.0, 200.0));
				count++;
			}
		}
		else if(carSeableName.equals("三菱")){
			for(int i=1;i<10;i++){
				carList.add(new CarInfor(i, "Z", "test", "test", 200.0, "S"+i, 20.0, 100.0, 200.0));
				count++;
		}
		}
